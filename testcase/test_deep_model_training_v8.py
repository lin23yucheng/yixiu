...
@allure.feature("场景：综合-目标检测YoloV8训练全流程")
class TestDeepModelTraining:
    ...
    def setup_class(cls):
        ...
        cls.task_name = f"接口自动化-{time_str}-目标检测V8"  # 统一任务名称格式
    ...
    def test_deep_task_workflow_v8(self):
        ...
        with allure.step("步骤5：开始目标检测YoloV8模型训练"):
        ...
        allure.dynamic.description(
            "深度（目标检测）Yolov8模型训练&后处理&部署测试完成！\n"
            f"总耗时: {time.strftime('%H:%M:%S', time.gmtime(time.time() - total_start))}"
        )
        print("\n\n\033[92m深度（目标检测）-自动化Yolov8模型训练&后处理&部署测试完成！\033[0m")
